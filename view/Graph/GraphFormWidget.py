...
from view.Graph.GraphGUI import GraphGUI

class OptionsForm(QGroupBox):

    def __init__(self, name, options, parent=None) -> None:
        ...
        optionsLayout.addRow(f"Positioning randomness ({int(self.options.POINTS_DISTANCE_RANDOMNESS*100)}%)", self.randomnessEdit)
        ...
        def changeRadius():
            try:
                self.options.POINTS_RADIUS = int(self.radiusEdit.value())
            except:
                self.options.POINTS_RADIUS = DEFAULT_POINTS_RADIUS
            self.parent().parent().pygameWidget.repaint()  

        def changeIterations():
            try:
                self.options.POINTS_POSITIONING_ITERATIONS = int(self.iterationsEdit.value())
            except:
                self.options.POINTS_POSITIONING_ITERATIONS = DEFAULT_POINTS_POSITIONING_ITERATIONS

        def changeDistance():
            try:
                self.options.POINTS_DISTANCE_RANDOMNESS = round(self.randomnessEdit.value()/200,2)
            except:
                self.options.POINTS_DISTANCE_RANDOMNESS = DEFAULT_POINTS_DISTANCE_RANDOMNESS   
            optionsLayout.labelForField(self.randomnessEdit).setText(f"Positioning randomness ({int(self.options.POINTS_DISTANCE_RANDOMNESS*100)}%)")   
        ...
        self.randomnessEdit.setValue(self.options.POINTS_DISTANCE_RANDOMNESS*200)
        
        self.iterationsEdit.setMinimum(0)
        self.iterationsEdit.setMaximum(1000)
        self.iterationsEdit.setValue(self.options.POINTS_POSITIONING_ITERATIONS)

        self.radiusEdit.setValue(self.options.POINTS_RADIUS)
...
class GraphFormWidget(QtWidgets.QWidget):   
    def __init__(self, options, parent=None):
        super(GraphFormWidget,self).__init__(parent)
        self.options = options
    
        #######################################

        self.optionsGroup = OptionsForm("Options", self.options, self)
        self.graphGroup = RandomizerForm(self.options, self)
