...
class Subject:
    ...
    def __init__(self, duration, amount, group, prof=None, subject=None):
...
class Course:
    ...
    def __init__(self, day, start, duration, group, prof, subject):
        self.day = day
        self.start = start
        self.duration = duration
        self.end = None
    ...
    def generate_diary (self,school):
        ...
        for i in graph_sche.vertices:
            if i.color in colors_hours:
                ...
                self.all_diaries[i.value.group][colors_hours[i.color][0]][colors_hours[i.color][1]] = i
                self.all_diaries[i.value.group][colors_hours[i.color][0]][colors_hours[i.color][1]+1] = i
                if (i.value.duration==2):
                    self.all_diaries[i.value.group][colors_hours[i.color][0]][colors_hours[i.color][1]+2] = i
                    self.all_diaries[i.value.group][colors_hours[i.color][0]][colors_hours[i.color][1]+3] = i
            else:
                place = False
                for l in range(5):
                    for j in [0,5,11,16]:
                        if (l,j) not in colors_hours.values() and j not in [4,9,10,15] and not place:
                            ...
                            i.value.start = j
                            i.value.end = i.value.start+i.value.duration
                            self.all_diaries[i.value.group][l][j] = i
                            self.all_diaries[i.value.group][l][j+1] = i
                            if (i.value.duration==2):
                                self.all_diaries[i.value.group][l][j+2] = i
                                self.all_diaries[i.value.group][l][j+3] = i
                            place = True
...
def generate_schedule_graph_from_subjects(subjects):
    ...
    for i,c in enumerate(subjects):
        for j in range(c.amount):
            vertices.add(Vertex(str(i+1)+"_"+str(j+1), Course(None,None,c.duration,c.group,c.prof,c.subject), 'c'+str(i+1)+'_'+str(j+1)))
    lV = list(vertices)
    # print(lV)
...
def main_schedule():
    ...
    import res.schedule.data_schedule as dsh
